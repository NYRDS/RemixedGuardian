...
class Session:
    ...
    def user_text(self, user_text:str, user_name:str):
        self.data[HISTORY].append({ROLE: 'user', CONTENT: self.turn_string(user_name, user_text)})

    def llm_text(self, llm_text:str):
        self.data[HISTORY].append({ROLE: 'assistant', CONTENT: self.turn_string(ai.name, llm_text)})
